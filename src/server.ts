...
import { JSONRPCMessage } from "@modelcontextprotocol/sdk/types.js"; // Import JSONRPCMessage from types path
import {
  z,
  ZodTypeAny,
  ZodObject,
  ...
  ZodEffects,
  ZodError,
} from "zod"; // Import Zod and necessary types
...
import {
  AgentRpc, // The core RPC interface type
  createAgentInterface, // Function to create the typed RPC proxy
  ModuleInfo, // Type for module information
  PointerOperation, // Enum/Type for pointer operations (if defined)
  PointerAuthConfig, // Type for pointer auth config
  PointerAuthKey, // Type for pointer auth key
  BatchOperationRequest, // Type for batch operation requests
  MemoryScanOptions, // Type for memory scan options
  MemoryAllocOptions, // Type for memory allocation options
  PageProtection, // Type for memory page protection
  ProcessInfo, // Type for process information
  ThreadDetails, // Type for thread details
  EnumerateRangesSpecifier, // Type for enumerating memory ranges
  SocketListenOptions, // Type for socket listen options
  SocketConnectOptions, // Type for socket connect options
  SocketConnection, // Type for socket connection info
  SocketListener, // Type for socket listener info
  StalkerFollowOptions, // Type for Stalker follow options
  StalkerMemoryRange, // Type for Stalker memory range
  StalkerCallProbeCallbackDefinition, // Type (if used) for Stalker callbacks
  StalkerConfig, // Type for Stalker config
  KernelScanOptions, // Type (if used) for kernel scanning
  InterceptorAttachConfig, // Type (if used) for interceptor attach config
  ReplacementConfig, // Type (if used) for replacement config
  BreakpointKind, // Type (if used) for breakpoint kinds
  MemoryRange, // Type for memory range details
  MemoryOperationResult, // Example result type, adjust if needed
...
export interface ServerConfig {
  transport: "stdio" | "sse";
  port?: number; // Port required only for SSE
  endpoint?: string; // Endpoint required only for SSE
}

// Enhance FridaClient interface/assumption to include EventEmitter properties
interface FridaClientLike extends EventEmitter {
  attach(target: string | number): Promise<void>;
  detach(): Promise<void>;
  getIsAttached(): boolean;
  on(
    event: "agentMessage",
    listener: (payload: any, data: Buffer | null) => void
  ): this;
  // Add other necessary methods if known (e.g., callRpc)
}

export class McpFridaServer {
  private server: McpServer;
  private fridaClient: FridaClientLike;
  ...
  private agentRpc?: AgentRpc | null = null;
  private fridaSession?: frida.Session | null = null;
  private fridaDevice?: frida.Device | null = null;
  private currentTarget: string | number | null = null;
  private isAttaching: boolean = false; // Prevent concurrent attach attempts
  ...
  constructor() {
    ...
    this.fridaClient = new FridaClient() as FridaClientLike;
  ...
  private setupEventBridge() {
    this.fridaClient.on("agentMessage", (payload, _data) => {
      if (this.currentConfig?.transport === "sse" && this.sseTransport) {
        logger.info(
          `Forwarding agent event via send():`,
  ...
  private onDetached(
    ...
    crash: frida.Crash | null
  ) {
    const pid = this.fridaSession?.pid; // Capture PID before nulling session
    logger.warn(
      `Session detached from PID ${pid ?? "unknown"}. Reason: ${reason}`,
      "Detach"
    );
    if (crash) {
    ...
    if (this.currentConfig?.transport === "sse" && this.sseTransport) {
      ...
      this.sseTransport
        ...
        .catch((err) =>
          logger.error(
            `Error sending detachment notification: ${err}`,
            "Detach"
          )
        );
    }
  ...
  private async checkAgentScript(): Promise<string> {
    try {
      ...
      logger.error(
        `Agent script not found or unreadable at: ${AGENT_SCRIPT_PATH}`,
        "Attach"
      );
      logger.error(
        `Please build the agent first (e.g., using 'pnpm run agent:build').`,
        "Attach"
      );
  ...
  public async attachToProcess(target: string | number): Promise<{
    success: boolean;
    message: string;
    pid?: number;
    ...
    if (this.fridaSession && !this.fridaSession.isDetached) {
      if (this.currentTarget === target || this.fridaSession.pid === target) {
        logger.info(
          `Already attached to target: ${target} (PID: ${this.fridaSession.pid}).`,
          "Attach"
        );
        return {
          success: true,
        ...
        logger.warn(
          `Already attached to a different process (PID: ${this.fridaSession.pid}). Detaching before attaching to ${target}.`,
          "Attach"
        );
    ...
    try {
      ...
      logger.info(
        `Attached successfully to PID: ${this.fridaSession.pid}`,
        "Attach"
      );

      this.fridaSession.detached.connect((reason, crash) =>
        this.onDetached(reason, crash)
      );
      logger.info(
        `Detached handler registered for PID: ${this.fridaSession.pid}`,
        "Attach"
      );
      ...
      script.message.connect((message: any, data: Buffer | null) => {
        logger.trace(
          `[Agent Message]: ${JSON.stringify(message)} ${
            data ? data.toString() : ""
          }`,
          "FridaClientBridge"
        );
      ...
      logger.error(
        `Failed during attach/inject to ${target}: ${error.message || error}`,
        "Attach"
      );
      if (this.fridaSession && !this.fridaSession.isDetached) {
        try {
  ...
  public async detachFromProcess(): Promise<{
    success: boolean;
    message: string;
  }> {
    if (this.fridaSession && !this.fridaSession.isDetached) {
      ...
      try {
        ...
        logger.error(
          `Error detaching from PID ${pid}: ${err.message || err}`,
          "Detach"
        );
        this.agentRpc = null;
        this.fridaSession = null;
  ...
  private _registerAllTools() {
    ...
    const createAgentToolImplementation = <T extends ZodRawShape>(
      name: string,
      implementationLogic: (
        params: z.objectOutputType<T, ZodTypeAny>,
        agentRpc: AgentRpc
      ) => Promise<any>
    ) => {
      ...
      return async (
        ...
        if (
          ...
          logger.error(
            `Attempted to call agent tool '${name}' while detached.`,
            "Tool"
          );
        ...
        try {
          ...
          logger.error(
            `Tool Error - ${name}, Params: ${JSON.stringify(
              params
            )}, Error: ${error}`,
            "Tool"
          );
    ...
    this.server.tool(
      "system_attachProcess",
      "附加到进程。可通过进程名称或进程ID进行附加。",
      {
        processName: z
          .string()
          .optional()
          .describe(
            "目标进程的名称 (例如 'PlantsVsZombies.exe')。与pid二选一。"
          ),
        pid: z
          .number()
          .int()
      ...
      async (params: {
        ...
        logger.info(
          `Executing tool: system_attachProcess with params: ${JSON.stringify(
            params
          )}`,
          "Tool"
        );
        ...
        try {
          // 根据提供的参数类型决定使用哪个目标
          const target =
            params.pid !== undefined ? params.pid : params.processName!;
          ...
          logger.error(
            `Tool Error - system_attachProcess, Params: ${JSON.stringify(
              params
            )}, Error: ${error}`,
            "Tool"
          );
    ...
    this.server.tool(
      "system_listAttachedSession",
      "获取当前 MCP 服务器附加的 Frida 会话信息。",
      {}, // No parameters
      async (): Promise<CallToolResult> => {
        ...
        try {
          ...
          logger.error(
            `Tool Error - system_listAttachedSession, Error: ${error}`,
            "Tool"
          );
          const errorMessage =
            error instanceof Error ? error.message : String(error);
    ...
    const consolidatedNativeReadParams: ZodRawShape = {
      ...
      maxLength: z
        ...
        .optional()
        .describe(
          "读取字符串 ('cstring', 'utf8', 'utf16', 'ansi') 时必需的最大长度/字节数。"
        ),
      // volatile: z.boolean().optional().describe("是否执行 volatile 读取 (主要用于 'bytearray')。")
    };
    this.server.tool(
      "native_read",
      "从指定地址读取指定类型的数据。根据 dataType 不同，可能需要 length 或 maxLength 参数。",
      consolidatedNativeReadParams,
      createAgentToolImplementation<typeof consolidatedNativeReadParams>(
        "native_read",
        async (params, agentRpc) => {
          const { address, dataType, length, maxLength } = params;
          let resultPromise: Promise<any>;
          switch (dataType) {
              case "pointer":   resultPromise = agentRpc.nativePointerReadPointer(address); break;
              case "s8":        resultPromise = agentRpc.nativePointerReadS8(address); break;
              case "u8":        resultPromise = agentRpc.nativePointerReadU8(address); break;
              case "s16":       resultPromise = agentRpc.nativePointerReadS16(address); break;
              case "u16":       resultPromise = agentRpc.nativePointerReadU16(address); break;
              case "s32":       resultPromise = agentRpc.nativePointerReadS32(address); break;
              case "u32":       resultPromise = agentRpc.nativePointerReadU32(address); break;
              case "s64":       resultPromise = agentRpc.nativePointerReadS64(address); break;
              case "u64":       resultPromise = agentRpc.nativePointerReadU64(address); break;
              case "short":     resultPromise = agentRpc.nativePointerReadShort(address); break;
              case "ushort":    resultPromise = agentRpc.nativePointerReadUShort(address); break;
              case "int":       resultPromise = agentRpc.nativePointerReadInt(address); break;
              case "uint":      resultPromise = agentRpc.nativePointerReadUInt(address); break;
              case "long":      resultPromise = agentRpc.nativePointerReadLong(address); break;
              case "ulong":     resultPromise = agentRpc.nativePointerReadULong(address); break;
              case "float":     resultPromise = agentRpc.nativePointerReadFloat(address); break;
              case "double":    resultPromise = agentRpc.nativePointerReadDouble(address); break;
              case "bytearray":
                  if (length === undefined) throw new Error("`length` is required when dataType is 'bytearray'");
                  resultPromise = agentRpc.nativePointerReadByteArray(address, length); break;
              case "cstring":
                  if (maxLength === undefined) throw new Error("`maxLength` is required for 'cstring'");
                  resultPromise = agentRpc.nativePointerReadCString(address, maxLength); break;
              case "utf8":
                  if (maxLength === undefined) throw new Error("`maxLength` is required for 'utf8'");
                  resultPromise = agentRpc.nativePointerReadUtf8String(address, maxLength); break;
              case "utf16":
                  if (maxLength === undefined) throw new Error("`maxLength` is required for 'utf16'");
                  resultPromise = agentRpc.nativePointerReadUtf16String(address, maxLength); break;
              case "ansi":
                  if (maxLength === undefined) throw new Error("`maxLength` is required for 'ansi'");
                  resultPromise = agentRpc.nativePointerReadAnsiString(address, maxLength); break;
    ...
    const consolidatedNativeWriteParams: ZodRawShape = {
      ...
      value: z
        .any()
        .describe(
          "要写入的值。类型应匹配 dataType (s64/u64/long/ulong 推荐字符串, bytearray 需数字数组)。"
        ),
      // volatile: z.boolean().optional().describe("是否执行 volatile 写入 (主要用于 'bytearray')。")
    };
    this.server.tool(
      "native_write",
      "向指定地址写入指定类型的数据。value 的类型需匹配 dataType。",
      consolidatedNativeWriteParams,
      createAgentToolImplementation<typeof consolidatedNativeWriteParams>(
        "native_write",
        async (params, agentRpc) => {
          ...
          let resultPromise: Promise<any>;
          switch (dataType) {
              case "pointer":   resultPromise = agentRpc.nativePointerWritePointer(address, value); break;
              case "s8":        resultPromise = agentRpc.nativePointerWriteS8(address, value); break;
              case "u8":        resultPromise = agentRpc.nativePointerWriteU8(address, value); break;
              case "s16":       resultPromise = agentRpc.nativePointerWriteS16(address, value); break;
              case "u16":       resultPromise = agentRpc.nativePointerWriteU16(address, value); break;
              case "s32":       resultPromise = agentRpc.nativePointerWriteS32(address, value); break;
              case "u32":       resultPromise = agentRpc.nativePointerWriteU32(address, value); break;
              case "s64":       resultPromise = agentRpc.nativePointerWriteS64(address, value); break;
              case "u64":       resultPromise = agentRpc.nativePointerWriteU64(address, value); break;
              case "short":     resultPromise = agentRpc.nativePointerWriteShort(address, value); break;
              case "ushort":    resultPromise = agentRpc.nativePointerWriteUShort(address, value); break;
              case "int":       resultPromise = agentRpc.nativePointerWriteInt(address, value); break;
              case "uint":      resultPromise = agentRpc.nativePointerWriteUInt(address, value); break;
              case "long":      resultPromise = agentRpc.nativePointerWriteLong(address, value); break;
              case "ulong":     resultPromise = agentRpc.nativePointerWriteULong(address, value); break;
              case "float":     resultPromise = agentRpc.nativePointerWriteFloat(address, value); break;
              case "double":    resultPromise = agentRpc.nativePointerWriteDouble(address, value); break;
              case "bytearray": resultPromise = agentRpc.nativePointerWriteByteArray(address, value); break;
              case "utf8":      resultPromise = agentRpc.nativePointerWriteUtf8String(address, value); break;
              case "utf16":     resultPromise = agentRpc.nativePointerWriteUtf16String(address, value); break;
              case "ansi":      resultPromise = agentRpc.nativePointerWriteAnsiString(address, value); break;
    ...
    const consolidatedNativeOperateParams = {
      ...
      operand: z
        ...
        .optional()
        .describe(
          "第二个操作数 (用于 add, sub, and, or, xor, shl, shr, compare, equals, blend)，通常为字符串表示。"
        ),
      radix: z
        .number()
        .int()
    ...
    this.server.tool(
      "native_operate",
      "对指针地址执行算术、位运算、比较、转换或认证操作。根据 operation 不同，可能需要 operand, radix, authConfig, authKey 或 blendOperand 参数。",
      consolidatedNativeOperateParams,
      createAgentToolImplementation<typeof consolidatedNativeOperateParams>(
        "native_operate",
        async (params, agentRpc) => {
          const { operation, address, operand, radix, authConfig, authKey, blendOperand } = params;
          let resultPromise: Promise<any>;
          switch (operation) {
              case "add": case "sub": case "shl": case "shr": case "compare":
                  if (operand === undefined) throw new Error(`\`operand\` is required for operation '${operation}'`);
                  let opValueNum: string | number = operand;
                  if (!isNaN(Number(operand))) { opValueNum = Number(operand); }
                  if (operation === "add") resultPromise = agentRpc.nativePointerAdd(address, opValueNum);
                  else if (operation === "sub") resultPromise = agentRpc.nativePointerSub(address, opValueNum);
                  else if (operation === "shl") resultPromise = agentRpc.nativePointerShl(address, opValueNum as number);
                  else if (operation === "shr") resultPromise = agentRpc.nativePointerShr(address, opValueNum as number);
                  else resultPromise = agentRpc.nativePointerCompare(address, opValueNum);
                  break;
              case "and": case "or": case "xor": case "equals":
                  if (operand === undefined) throw new Error(`\`operand\` is required for operation '${operation}'`);
                  if (operation === "and") resultPromise = agentRpc.nativePointerAnd(address, operand);
                  else if (operation === "or") resultPromise = agentRpc.nativePointerOr(address, operand);
                  else if (operation === "xor") resultPromise = agentRpc.nativePointerXor(address, operand);
                  else resultPromise = agentRpc.nativePointerEquals(address, operand);
                  break;
               case "blend":
                   if (blendOperand === undefined) throw new Error("`blendOperand` is required for operation 'blend'");
                   resultPromise = agentRpc.nativePointerBlend(address, blendOperand); break;
              case "not": resultPromise = agentRpc.nativePointerNot(address); break;
              case "isNull": resultPromise = agentRpc.nativePointerIsNull(address); break;
              case "toInt32": resultPromise = agentRpc.nativePointerToInt32(address); break;
              case "toUint32": resultPromise = agentRpc.nativePointerToUInt32(address); break;
              case "toMatchPattern": resultPromise = agentRpc.nativePointerToMatchPattern(address); break;
              case "toString":
                  if (radix === undefined) throw new Error("`radix` is required for operation 'toString'");
                  resultPromise = agentRpc.nativePointerToString(address, radix); break;
              case "sign":
                  if (authConfig === undefined) throw new Error("`authConfig` is required for operation 'sign'");
                  resultPromise = agentRpc.nativePointerSign(address, authConfig); break;
              case "strip":
                  if (authKey === undefined) throw new Error("`authKey` is required for operation 'strip'");
                  resultPromise = agentRpc.nativePointerStrip(address, authKey); break;
    ...
    this.server.tool(
      "native_createPointer",
      "根据给定的地址字符串创建一个指针表示。",
      consolidatedNativeCreateParams,
      createAgentToolImplementation<typeof consolidatedNativeCreateParams>(
        "native_createPointer",
        async (params, agentRpc) => {
    ...
    this.server.tool(
      "native_batchOperate",
      "(高级) 在一次 RPC 调用中执行多个指针操作 (Add, Subtract, And, Or, Xor, Shl, Shr)。",
      consolidatedNativeBatchParams,
      createAgentToolImplementation<typeof consolidatedNativeBatchParams>(
        "native_batchOperate",
        async (params, agentRpc) => {
          // Assuming agent expects a single object
          const request = {
            operations: params.operations as unknown as PointerOperation[],
            addresses: params.addresses,
            operands: params.operands,
          };
          // Agent side needs to handle parsing string operands if necessary
          return await agentRpc.nativePointerBatchOperate(request as any);
        }
      )
    );
    ...
    const memoryScanParams = {
      ...
      pattern: z
        .string()
        .describe(
          "要扫描的内存模式 (例如 '01 ?? cd ef' 或 Frida 模式字符串)。"
        ),
      sync: z
        .boolean()
        .optional()
    ...
    this.server.tool(
      "memory_scan",
      "在指定内存区域扫描特定模式。可选择同步或异步执行。",
      memoryScanParams,
      createAgentToolImplementation<typeof memoryScanParams>(
        "memory_scan",
        async (params, agentRpc) => {
          // 构建符合 MemoryScanOptions 接口的选项对象
          const options: MemoryScanOptions = {};
          ...
          if (params.sync) {
            logger.info(
              `执行同步内存扫描，地址: ${params.baseAddress}, 大小: ${params.size}`,
              "Tool"
            );
            return await agentRpc.memoryScanSync(
              params.baseAddress,
              params.size,
              params.pattern,
              options
            ...
            // 否则，使用异步扫描
            logger.info(
              `执行异步内存扫描，地址: ${params.baseAddress}, 大小: ${params.size}`,
              "Tool"
            );
            return await agentRpc.memoryScan(
              params.baseAddress,
              params.size,
              params.pattern,
              options
    ...
    this.server.tool(
      "memory_alloc",
      "智能内存分配：可分配原始内存块、字符串或字节数组。",
      memoryAllocParams,
      createAgentToolImplementation<typeof memoryAllocParams>(
        "memory_alloc",
        async (params, agentRpc) => {
          // 根据字符串类型参数决定分配方式
          if (params.stringType !== "none") {
            ...
            if (!params.text) {
              throw new Error(
                `参数'text'在stringType='${params.stringType}'时是必需的`
              );
            }
            ...
            // 原始内存分配模式
            // 构建内存分配选项
            const allocOptions: MemoryAllocOptions = {};

            if (params.options) {
              if (params.options.near) allocOptions.near = params.options.near;
    ...
    this.server.tool(
      ...
      "从一个内存地址复制数据到另一个地址。",
      memoryCopyParams,
      createAgentToolImplementation<typeof memoryCopyParams>(
        "memory_copy",
        async (params, agentRpc) => {
          return await agentRpc.memoryCopy(
            params.destination,
            params.source,
            params.size
          );
    ...
    this.server.tool(
      "memory_dup",
      "复制指定的内存区域并返回新分配区域的地址。",
      memoryDupParams,
      createAgentToolImplementation<typeof memoryDupParams>(
        "memory_dup",
        async (params, agentRpc) => {
    ...
    const memoryProtectParams = {
      ...
      protection: z
        ...
        .regex(/^[r-][w-][x-]$/)
        .describe(
          "新的内存保护属性 (格式: 'rwx', 'rw-', 'r-x', 'r--', '-wx', '-w-', '--x', '---')。"
        ),
    };
    this.server.tool(
      "memory_protect",
      "修改指定内存区域的保护属性。",
      memoryProtectParams,
      createAgentToolImplementation<typeof memoryProtectParams>(
        "memory_protect",
        async (params, agentRpc) => {
          // Cast protection to PageProtection if the agent expects it
          return await agentRpc.memoryProtect(
            params.address,
            params.size,
            params.protection as PageProtection
          );
        }
      )
    );
    ...
    this.server.tool(
      "memory_queryProtection",
      "查询指定内存地址的保护属性。",
      memoryQueryProtectionParams,
      createAgentToolImplementation<typeof memoryQueryProtectionParams>(
        "memory_queryProtection",
        async (params, agentRpc) => {
    ...
    this.server.tool(
      "memory_patchCode",
      "向指定内存地址写入字节码补丁。",
      memoryPatchCodeParams,
      createAgentToolImplementation<typeof memoryPatchCodeParams>(
        "memory_patchCode",
        async (params, agentRpc) => {
          // Assuming agent method name matches and handles the 'apply' flag if needed
          return await agentRpc.memoryPatchCode(
            params.address,
            params.patchBytes /*, params.apply */
          );
        }
      )
    );
    ...
    this.server.tool(
      "process_getInfo",
      "获取目标进程的基本信息 (例如架构、平台、页面大小)。",
      {}, // No parameters
      createAgentToolImplementation<{}>(
        "process_getInfo",
        async (_params, agentRpc) => { // Use _params since it's empty
    ...
    this.server.tool(
        "process_getDirs",
        "获取与进程相关的常见目录路径 (当前、主目录、临时目录)。",
        {}, // No parameters
        createAgentToolImplementation<{}>(
            "process_getDirs",
            async (_params, agentRpc) => {
    ...
    this.server.tool(
        "process_findModule",
        "查找模块。可以通过内存地址或模块名称进行查找 (二选一)。",
        processFindModuleParams,
        createAgentToolImplementation<typeof processFindModuleParams>(
            "process_findModule",
            async (params, agentRpc) => {
    ...
    this.server.tool(
        "process_findRangeByAddress",
        "根据内存地址查找包含该地址的内存范围。",
        processFindRangeByAddressParams,
        createAgentToolImplementation<typeof processFindRangeByAddressParams>(
            "process_findRangeByAddress",
            async (params, agentRpc) => {
    ...
    this.server.tool(
        "process_enumerate",
        "枚举进程中的对象，例如模块、内存范围或 malloc 分配的范围。",
        baseProcessEnumerateParams.shape, // <<< 使用基础对象的 shape
        createAgentToolImplementation<typeof baseProcessEnumerateParams.shape>( // 类型推断也使用基础对象的 shape
            "process_enumerate",
            async (params, agentRpc) => {
                 ...
                 switch (params.type) {
                     ...
                     case "ranges":
                         ...
                         const spec: EnumerateRangesSpecifier = {};
                         if (params.specifier !== 'all') {
                             spec.protection = params.specifier! as PageProtection; // 使用非空断言
                         }
                     ...
                     default:
                         ...
                         const exhaustiveCheck: never = params.type; // 用于编译时检查
    ...
    this.server.tool(
        "stalker_setFollow",
        "开始或停止 Stalker 跟踪指定线程。提供 options 参数以开始跟踪，不提供则停止跟踪。",
        stalkerSetFollowParams,
        createAgentToolImplementation<typeof stalkerSetFollowParams>(
            "stalker_setFollow",
            async (params, agentRpc) => {
                if (params.options) {
                    // 开始跟踪
                    const followOptions: StalkerFollowOptions = {
    ...
    this.server.tool(
        "stalker_excludeRange",
        "将指定的内存范围从 Stalker 检测中排除。",
        stalkerExcludeRangeParams,
        createAgentToolImplementation<typeof stalkerExcludeRangeParams>(
            "stalker_excludeRange",
            async (params, agentRpc) => {
                const range: StalkerMemoryRange = {
    ...
    this.server.tool(
        "stalker_manageEvents",
        "获取或清除指定线程累积的 Stalker 事件。",
        stalkerManageEventsParams,
        createAgentToolImplementation<typeof stalkerManageEventsParams>(
            "stalker_manageEvents",
            async (params, agentRpc) => {
    ...
    this.server.tool(
        "stalker_flush",
        "刷新当前执行 Agent RPC 的线程的 Stalker 事件队列 (注意: 作用于 Agent 内部线程)。",
        {}, // No parameters
        createAgentToolImplementation<{}> (
            "stalker_flush",
            async (_params, agentRpc) => {
    ...
    this.server.tool(
        "stalker_garbageCollect",
        "触发 Stalker 的内部垃圾回收机制。",
        {}, // No parameters
        createAgentToolImplementation<{}> (
            "stalker_garbageCollect",
            async (_params, agentRpc) => {
    ...
    this.server.tool(
        "stalker_manageConfig",
        "获取或设置 Stalker 的配置参数。提供 config 参数以设置，省略则获取。",
        stalkerManageConfigParams,
        createAgentToolImplementation<typeof stalkerManageConfigParams>(
            "stalker_manageConfig",
            async (params, agentRpc) => {
                if (params.config) {
                    // 设置配置
                    const newConfig: StalkerConfig = {};
                    if (params.config.trustThreshold !== undefined) newConfig.trustThreshold = params.config.trustThreshold;
                    if (params.config.queueCapacity !== undefined) newConfig.queueCapacity = params.config.queueCapacity;
                    if (params.config.queueDrainInterval !== undefined) newConfig.queueDrainInterval = params.config.queueDrainInterval;
    ...
    this.server.tool(
        "kernel_enumerate",
        "枚举内核对象：模块、内存范围或特定模块的内存范围。根据 type 不同，可能需要 protection 和 moduleName 参数。",
        baseKernelEnumerateParams.shape, // 使用基础 shape
        createAgentToolImplementation<typeof baseKernelEnumerateParams.shape>(
            "kernel_enumerate",
            async (params, agentRpc) => {
                ...
                switch (params.type) {
                    ...
                    case "ranges":
                        ...
                        return await agentRpc.kernelEnumerateRanges(params.protection! as PageProtection);
                    case "moduleRanges":
                        // refine 保证了 protection 存在
                        logger.info(`Enumerating ranges for kernel module ${params.moduleName ?? 'all'} with protection ${params.protection}`, "Tool");
                        return await agentRpc.kernelEnumerateModuleRanges(params.moduleName ?? null, params.protection! as PageProtection);
                    default:
    ...
    this.server.tool(
        ...
        "在内核空间分配内存。",
        kernelAllocParams,
        createAgentToolImplementation<typeof kernelAllocParams>(
            "kernel_alloc",
            async (params, agentRpc) => {
    ...
    this.server.tool(
        ...
        "修改内核内存区域的保护属性。",
        kernelProtectParams,
        createAgentToolImplementation<typeof kernelProtectParams>(
            "kernel_protect",
            async (params, agentRpc) => {
                logger.info(`Setting kernel protection for ${params.address} (${params.size} bytes) to ${params.protection}`, "Tool");
                return await agentRpc.kernelProtect(params.address, params.size, params.protection as PageProtection);
            }
        )
    );
    ...
    this.server.tool(
        "kernel_scan",
        "在指定的内核内存区域扫描特定模式。可选择同步或异步执行。",
        kernelScanParams,
        createAgentToolImplementation<typeof kernelScanParams>(
            "kernel_scan",
            async (params, agentRpc) => {
    ...
    const kernelReadParams = {
     ...
     this.server.tool(
      ...
      "从指定的内核内存地址读取指定类型的数据。",
      kernelReadParams,
      createAgentToolImplementation<typeof kernelReadParams>(
        "kernel_read",
        async (params, agentRpc) => {
          const { address, dataType, lengthOrSize } = params;
          let resultPromise: Promise<any>;
          switch(dataType) {
              case "s8": resultPromise = agentRpc.kernelReadS8(address); break;
              case "u8": resultPromise = agentRpc.kernelReadU8(address); break;
              case "s16": resultPromise = agentRpc.kernelReadS16(address); break;
              case "u16": resultPromise = agentRpc.kernelReadU16(address); break;
              case "s32": resultPromise = agentRpc.kernelReadS32(address); break;
              case "u32": resultPromise = agentRpc.kernelReadU32(address); break;
              case "float": resultPromise = agentRpc.kernelReadFloat(address); break;
              case "double": resultPromise = agentRpc.kernelReadDouble(address); break;
              case "bytearray":
                  if (lengthOrSize === undefined) throw new Error("`lengthOrSize` is required for 'bytearray'");
                  resultPromise = agentRpc.kernelReadByteArray(address, lengthOrSize); break;
              case "cstring":
                  if (lengthOrSize === undefined) throw new Error("`lengthOrSize` is required for 'cstring'");
                  resultPromise = agentRpc.kernelReadCString(address, lengthOrSize); break;
              case "utf8":
                  if (lengthOrSize === undefined) throw new Error("`lengthOrSize` is required for 'utf8'");
                  resultPromise = agentRpc.kernelReadUtf8String(address, lengthOrSize); break;
              case "utf16":
                  if (lengthOrSize === undefined) throw new Error("`lengthOrSize` is required for 'utf16'");
                  resultPromise = agentRpc.kernelReadUtf16String(address, lengthOrSize); break;
    ...
    this.server.tool(
      ...
      "向指定的内核内存地址写入指定类型的数据。",
      kernelWriteParams,
      createAgentToolImplementation<typeof kernelWriteParams>(
        "kernel_write",
        async (params, agentRpc) => {
          const { address, dataType, value } = params;
          // ... (类型验证)
          let resultPromise: Promise<any>;
          switch(dataType) {
              case "s8": resultPromise = agentRpc.kernelWriteS8(address, value); break;
              case "u8": resultPromise = agentRpc.kernelWriteU8(address, value); break;
              case "s16": resultPromise = agentRpc.kernelWriteS16(address, value); break;
              case "u16": resultPromise = agentRpc.kernelWriteU16(address, value); break;
              case "s32": resultPromise = agentRpc.kernelWriteS32(address, value); break;
              case "u32": resultPromise = agentRpc.kernelWriteU32(address, value); break;
              case "float": resultPromise = agentRpc.kernelWriteFloat(address, value); break;
              case "double": resultPromise = agentRpc.kernelWriteDouble(address, value); break;
              case "bytearray": resultPromise = agentRpc.kernelWriteByteArray(address, value); break;
              case "utf8": resultPromise = agentRpc.kernelWriteUtf8String(address, value); break;
              case "utf16": resultPromise = agentRpc.kernelWriteUtf16String(address, value); break;
  ...
  private async startSSEServer(port: number, endpoint: string): Promise<void> {
    this.app = express();
    ...
    this.app.get(endpoint, async (req: Request, res: Response) => {
      ...
      if (this.sseTransport) {
        logger.warn(
          "Closing previous SSE transport due to new connection.",
          "SSE"
        );
        await this.sseTransport
          .close()
          .catch((err: any) =>
            logger.error(`Error closing previous SSE transport: ${err}`, "SSE")
          );
      }
      ...
      req.on("close", async () => {
        ...
        if (this.sseTransport) {
          try {
            ...
            logger.error(
              `Error closing transport on disconnect: ${err}`,
              "SSE"
            );
          }
      ...
      try {
        ...
        logger.error(
          `Error connecting MCP server to SSE transport: ${err}`,
          "SSE"
        );
        if (!res.headersSent) {
    ...
    this.app.post(
      ...
      async (req: Request, res: Response) => {
        if (this.sseTransport) {
          ...
          logger.warn(
            "Received POST message but no active SSE transport.",
            "SSE"
          );
          res
            .status(400)
    ...
    await new Promise<void>((resolve, reject) => {
      this.httpServer?.listen(port, () => {
        ...
        logger.info(
          `SSE Endpoint Ready at http://localhost:${port}${endpoint}`,
          "SSE"
        );
        resolve();
      });
  ...
  // Modify start method
  public async start(config: ServerConfig) {
    this.currentConfig = config;
    logger.info(
      `Starting MCP Server with transport: ${config.transport}`,
      "Server"
    );
    await this.stop();
    try {
  ...
  public async stop() {
    logger.info(
      "Attempting to stop server and detach Frida session...",
      "Server"
    );
    let hadError = false;
    try {
